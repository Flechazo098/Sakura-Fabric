            ...
            var fluidSlot = builder.addSlot(RecipeIngredientRole.INPUT, 1, 1)
                    ...
                    .addTooltipCallback((recipeSlotView, tooltip) -> {
                        for (int i = 0; i < tooltip.size(); i++) {
                            ...
                            if (text.contains("mB")) {
                                tooltip.set(i, Component.literal(recipe.getRequiredFluid().getRequiredAmount() + " droplets / " + FermenterBlockEntity.TANK_CAPACITY + " droplets").withStyle(ChatFormatting.GRAY));
                                break;
                            }
                        }
                    });
            ...
            builder.addSlot(RecipeIngredientRole.OUTPUT, 93, 1)
                    ...
                    .addTooltipCallback((recipeSlotView, tooltip) -> {
                        for (int i = 0; i < tooltip.size(); i++) {
                            ...
                            String text = component.getString();
                            if (text.contains("mB")) {
                                tooltip.set(i, Component.literal(resultFluid.getAmount() + " droplets / " + FermenterBlockEntity.TANK_CAPACITY + " droplets").withStyle(ChatFormatting.GRAY));
                                break;
                            }
                        }
                    })
